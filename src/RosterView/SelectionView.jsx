...
function getRulesJSX(rules: Rule[]): Element<"table"> {
  ...
  const rulesRows = rules.map(rule =>
    createTableRow([rule.name, rule.description])
  );
...
function getKeywordsJSX(categories: Category[]): Element<"table"> {
  const categoriesListString = categories
    .map(category => category.name)
    .join(", ");
  return createTable(createTableHeader([KEYWORDS_TITLE]), [
    createTableRow([categoriesListString])
  ]);
}
...
function getModelsJSX(subselections: Selection[]): Element<"table"> | null {
  const models = subselections
    .filter(selection => selection.type === "model")
    .map(model => {
  ...
  if (models.length > 0) {
    return createTable(
      createTableHeader([MODELS_TITLE, SELECTIONS_TITLE]),
      models
    );
  } else {
    return null;
...
function getProfilesJSX(selection: Selection): { [string]: Element<"table"> } {
  ...
  const profilesGroupedByType: { [string]: Profile[] } = _.groupBy(
    allProfiles,
    "type"
  );
  ...
  return _.mapValues(profilesGroupedByType, (profiles, profileType) => {
    ...
    const uniqueProfiles = _.uniqBy(
      profilesGroupedByType[profileType],
    ...
    return (
      <div id={"datacard-" + id} className="datacard" key={"selection-" + id}>
        <div className="datacard--header">
          {name}
          {/* [{costsValues.points} points / {costsValues.powerLevel}PL]*/}
        </div>
        <div className="datacard--body">{orderedSectionsJSX}</div>
      </div>
    );
  }
}
